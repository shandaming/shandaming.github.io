...
<!--more-->
Today, I looked at the computer problem and found that there was a problem with the SSD hard disk, which led to the inability to find the uuid. My previous conclusion was wrong. Now, just install the system on another hard disk.
* There was no in-depth study of the previous issues, but only superficial phenomena. In the future, we must do in-depth research and study.

今天看了下电脑问题，发现是SSD硬盘有问题，然后导致找不到uuid，我之前的结论是错误的，现在将系统安装在另一个硬盘上就好了。
* 之前问题没有深入的研究，只是表面现象。以后做事必须要深入的研究和钻研。
