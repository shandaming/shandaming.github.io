...
<!--more-->
Today modified code found that I have some insights for the development, for the function well made into a level of abstraction, if there are multiple classes are using a method, it is best to also make a virtual function to reduce coupling. For business functions need to sort out the detailed requirements, divided into several small steps, for scene switching, there should be a scene into the initialization, leaving the end of the steps.

今天修改代码发现我对于开发有了些心得，对于功能做好做成抽象层次，如果有多个类都用一个方法，最好也做成虚函数，减少耦合。对于业务功能需要理清详细的需求，划分出几个小步骤，对于场景切换，应该有场景进入时的初始化，离开时的结束步骤。
